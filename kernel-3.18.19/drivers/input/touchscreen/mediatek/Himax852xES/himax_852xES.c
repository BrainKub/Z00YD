...
//dingyisheng 20160407 begin
//auto FW_UPDATE select among  BOE, HLT and other modules.
...
static char self_test_inter_flag=0;

/* //liupeng 20160127 begin
...
static int himax_input_register(struct himax_ts_data *ts)
{
	...
	set_bit(KEY_F21, ts->input_dev->keybit);
	...
	if(IC_TYPE == HX_85XX_ES_SERIES_PWON)
		{
			...
			//qiumeng 20160513 begin
			HX_RX_NUM				= 24;
			HX_TX_NUM				= 10; //modified by hebiao at 20160601
			HX_BT_NUM				= 0;  //modified by hebiao at 20160601
			HX_X_RES				= 720;
			HX_Y_RES				= 1280;
			HX_MAX_PT				= 5;
			HX_XY_REVERSE			= false;
			...
			Is_2T2R 				= true;
			HX_RX_NUM_2 			= 12;
			HX_TX_NUM_2 			= 20;	//modified by hebiao at 20160601
			//qiumeng 20160513 end
		...
		//qiumeng 20160513 begin
			HX_RX_NUM				= 24;  //0 //12
			HX_TX_NUM				= 10;  //0 //19 //modified by hebiao at 20160601
			HX_BT_NUM				= 0;   //0 //modified by hebiao at 20160601
			HX_X_RES				= 720; //0
			HX_Y_RES				= 1280;//0
			HX_MAX_PT				= 5;   //0
			HX_XY_REVERSE			= false; 
			HX_INT_IS_EDGE			= true; //false
			Is_2T2R 				= true;
			HX_RX_NUM_2 			= 12;
			HX_TX_NUM_2 			= 20;	//modified by hebiao at 20160601
		//qiumeng 20160513 end
...
//dingyisheng 20160407 begin
//complete moule_FW_update auto select among boe,helitai and other modules if we have. 
...
static int i_update_FW(void)
{
	...
	if(private_ts->vendor_sensor_id == 0x12) {
		ImageBuffer = i_CTPM_FW_BOE; //boe_module
		fullFileLength = sizeof(i_CTPM_FW_BOE); 
		printk("dingyisheng:BOE module update!");
	} else if (private_ts->vendor_sensor_id == 0x22) {
		ImageBuffer = i_CTPM_FW_HLT; //helitai_module
		fullFileLength = sizeof(i_CTPM_FW_HLT); 
		printk("dingyisheng:HLT module update!");
	}  else {
		ImageBuffer = NULL;  //unknown_module
	}
...
{
	unsigned char* ImageBuffer = NULL;//NULL ;//
...
//ranyanhao 20160323 begin
#ifdef CONFIG_WIND_DEVICE_INFO
	//Get TP type
	if(i2c_himax_read(client, 0xD1, data, 3, DEFAULT_RETRY_CNT) < 0){
		E("%s: i2c access fail!\n", __func__);
		return;
	}
		
	if(data[0] == 0x05 && data[1] == 0x85 &&
		(data[2] == 0x25 || data[2] == 0x26 || data[2] == 0x27 || data[2] == 0x28)){
			sprintf(g_ctp_id_str,"Hx%02X%02XE",data[1],data[2]);
	} else {
...
/*
 * The following wind_TP_info for E280L, 
*/
	g_ctp_fwvr = private_ts->vendor_config_ver;  //use config_ver as fw version.because every updata vendor_config_ver will be change!
	if(private_ts->vendor_sensor_id == 0x12) {
		g_ctp_vendor = 0x01;  //boyi_module
	} else if (private_ts->vendor_sensor_id == 0x22) {
		g_ctp_vendor = 0x02;  //helitai_module
	} else if (private_ts->vendor_sensor_id == 0x21) {
		g_ctp_vendor = 0x03;  //yassy_module
	} else {
		g_ctp_vendor = 0xFF;  //unknown_module
	}
#endif	
//ranyanhao 20160323 end
...
//							100);
					...
					input_report_key(ts->input_dev, KEY_MENU, 1);//yanglong modify  20151217 #92975
		}
...
//							100);
					...
					input_report_key(ts->input_dev, KEY_BACK, 1);		//gemingming KEY_APP_SWITCH->KEY_HOME #92975
		}
...
#endif
//qiumeng 20160418 start
#if defined(HX_USB_DETECT2)
	himax_cable_detect_func();
#endif
//qiumeng 20160418 end
#ifdef HX_CHIP_STATUS_MONITOR
		HX_CHIP_POLLING_COUNT=0;
			...
			if(vk_press == 0x00)
				{
					...
					//		input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
							...
							//input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
							//input_report_abs(ts->input_dev, ABS_MT_PRESSURE, w);
							printk("no_function station begin func==%s line=%d x=%d y=%d\n", __func__, __LINE__, x, y);
							input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
							input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *desc)
{
	...
	printk("no_function tpd_eint_interrupt_handler func==%s line=%d\n", __func__, __LINE__);
...
int ret_event = 0, KEY_EVENT = 0;	
	...
	printk("no_function touch_event_handler begin func==%s line=%d\n", __func__, __LINE__);

	do
	{
		set_current_state(TASK_INTERRUPTIBLE); 
		printk("no_function wait_event_interruptible begin func==%s line=%d\n", __func__, __LINE__);
		wait_event_interruptible(waiter,tpd_flag!=0);
		printk("no_function wait_event_interruptible end func==%s line=%d\n", __func__, __LINE__);
		...
		if (private_ts->debug_log_level & BIT(2)) {
...
#ifdef HX_SMART_WAKEUP
			printk("no_function gesture begin func==%s line=%d\n", __func__, __LINE__);
			if (atomic_read(&private_ts->suspend_mode)&&(!FAKE_POWER_KEY_SEND)&&(private_ts->SMWP_enable)) {
				...
				printk("ranyanhao ret_event = %d,KEY_EVENT = %d,SMWP_enable = %d\n",ret_event,KEY_EVENT,private_ts->SMWP_enable);
				if(ret_event)
					{
		...
		printk("no_function himax_ts_work begin func==%s line=%d\n", __func__, __LINE__);
		himax_ts_work();
		printk("no_function himax_ts_work end func==%s line=%d\n", __func__, __LINE__);
		himax_int_enable(private_ts->client->irq,1,false);
		printk("no_function himax_int_enable end func==%s line=%d\n", __func__, __LINE__);
	...
	printk("no_function kthread_stop func==%s line=%d\n", __func__, __LINE__);
...
static void himax_cable_detect_func(void)
{
	...
	//connect_status = 0;
	//qiumeng 0160418 start
	//connect_status = sprdchg_charger_is_adc();
	//qiumeng 20160427 begin
	//connect_status = gFG_Is_Charging;
	connect_status = bat_is_charger_exist();
	//qiumeng 20160427 end
...
static int himax_chip_self_test(void)
{
	uint8_t cmdbuf[11];
	uint8_t valuebuf[16];
	int i=0, pf_value=0x00, self_test_lop_counter = 0; //modified by hebiao at 20160601
...
//hebiao 20160601 begin 
		self_test_lop_counter++;
		if (self_test_lop_counter < 0x64)
//hebiao 20160601 end
	...
	size_t len, loff_t *pos)
{
	...
	//HX_PROC_SEND_FLAG = 0; //hebiao modified at 201661
	...
	if(!HX_PROC_SEND_FLAG)
	{
		...
		HX_PROC_SEND_FLAG=0;
	//modified by lizusheng 20160413 start
	self_test_inter_flag=0; //modified by hebiao 20160602
	printk("[HXTP] [Himax] self_test_inter_flag 2 =%d\n", self_test_inter_flag);
	//modified by lizusheng 20160413 end
	return ret;
}
...
//	himax_proc_reset_file = proc_create(HIMAX_PROC_RESET_FILE, (S_IWUSR), 
	himax_proc_reset_file = proc_create(HIMAX_PROC_RESET_FILE, 0666, 
...
//	himax_proc_register_file = proc_create(HIMAX_PROC_REGISTER_FILE, (S_IWUSR|S_IRUGO), 
	himax_proc_register_file = proc_create(HIMAX_PROC_REGISTER_FILE, 0666, 
...
printk("junpiao: dma_alloc_coherent\n");
	
	i2c_set_clientdata(client, ts);
	printk("frank_zhonghua1_himax_client-addr = %x\n",client->addr);
	ts->client = client;
...
printk("junpiao: i2c_set_clientdata\n");
...
//qiumeng 20160525 end
	...
	if(i_update_FW()==false) {
		...
		//ranyanhao 20160323 begin
		himax_read_TP_info(client);  //Read TP info again,when UPDATE_FW.
		//ranyanhao 20160323 end
	}
#endif
...
//ranyanhao 20160323
/*
#ifdef CONFIG_WIND_SMWP_ENABLE
	ts->SMWP_enable=1; 
	memset(ts->gesture_cust_en,1,16);
#else
	ts->SMWP_enable=0;
#endif
*/
...
#endif
//qiumeng 20160505 start
#if defined(HX_USB_DETECT2)
	himax_cable_detect_func();
#endif
//qiumeng 20160505 end
#ifdef HX_SMART_WAKEUP
	if(ts->SMWP_enable)
	{
		//Sense Off
